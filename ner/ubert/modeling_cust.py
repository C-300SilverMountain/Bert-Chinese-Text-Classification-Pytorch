    ...
    def encode(self, item, used_mask=False):
        ...
        for choice in item['choices']:
            try:
                ...
                textb = item['text']
                encode_dict = self.tokenizer.encode_plus(texta, textb, max_length=self.max_length,
                ...
                span_label_mask = np.zeros((self.max_length, self.max_length)) - 10000
                ...
                if item['task_type'] == '分类任务':
                    ...
                    for entity in choice['entity_list']:
                        ...
                        for entity_idx in entity_idx_list:
                            ...
                            start_idx_text_encode = self.tokenizer.encode(start_idx_text, add_special_tokens=False)
                            start_idx = question_len + len(start_idx_text_encode)

                            end_idx_text = item['text'][:entity_idx[1] + 1]
                            end_idx_text_encode = self.tokenizer.encode(end_idx_text, add_special_tokens=False)
        ...
        while len(input_ids1) < self.args.num_labels:
            ...
            span_labels_masks1.append(np.zeros((self.max_length, self.max_length)) - 10000)
        ...
        sample = {"input_ids": torch.tensor(input_ids).long(), "token_type_ids": torch.tensor(token_type_ids).long(),
            "attention_mask": torch.tensor(attention_mask).float(), "span_labels": torch.tensor(span_labels).float(),
...
class UbertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.query_layer = torch.nn.Sequential(torch.nn.Linear(in_features=self.config.hidden_size,
                                                               out_features=self.config.biaffine_size), torch.nn.GELU())
        self.key_layer = torch.nn.Sequential(torch.nn.Linear(in_features=self.config.hidden_size,
                                                             out_features=self.config.biaffine_size), torch.nn.GELU())
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, span_labels_mask=None,span_labels=None):
        ...
        span_logits = self.biaffine_query_key_cls(query, key).reshape(-1, num_label, seq_len, seq_len)
        ...
        if span_labels == None:
            ...
            soft_loss1 = self.loss_softmax(span_logits.reshape(-1, num_label, seq_len * seq_len),
                                           span_labels.reshape(-1, num_label, seq_len * seq_len))
...
class UbertLitModel(pl.LightningModule):
    ...
    def __init__(self, args, num_data=1):
        ...
        self.model = UbertModel.from_pretrained(self.args.pretrained_model_path)
        self.count = 0
    ...
    def training_step(self, batch, batch_idx):
        ...
        span_acc, recall, precise = self.comput_metrix_span(span_logits, batch['span_labels'])
        self.log('train_loss', loss)
        self.log('train_span_acc', span_acc)
        self.log('train_span_recall', recall)
        self.log('train_span_precise', precise)
        print()
        print('train_loss', loss.item(), 'train_span_acc', span_acc.item(), 'train_span_recall', recall.item(),
              'train_span_precise', precise.item())

        return loss

    def lr_scheduler_step(self, scheduler: LRSchedulerTypeUnion, optimizer_idx: int, metric: Optional[Any], ):
    ...
    def configure_optimizers(self):
        ...
        paras = [{'params': [p for n, p in paras if not any(nd in n for nd in no_decay)],
            'weight_decay': self.args.weight_decay},
            {'params': [p for n, p in paras if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}]
        optimizer = torch.optim.AdamW(paras, lr=self.args.learning_rate)
        scheduler = get_linear_schedule_with_warmup(optimizer, int(self.total_step * self.args.warmup), self.total_step)
...
class TaskModelCheckpoint:
    ...
    def __init__(self, args):
        self.callbacks = ModelCheckpoint(monitor=args.monitor, save_top_k=args.save_top_k, mode=args.mode,
                                         save_last=True, every_n_train_steps=args.every_n_train_steps,
                                         save_weights_only=args.save_weights_only, dirpath=args.checkpoint_path,
...
class extractModel:
    ...
    def extract_entity(self, text, entity_idx, text_start_id, text_mapping):
        ...
        end_split = text_mapping[entity_idx[1] - text_start_id] if (
                    entity_idx[1] - text_start_id < len(text_mapping) and entity_idx[1] - text_start_id >= 0) else []
        entity = ''
        if start_split != [] and end_split != []:
            entity = text[start_split[0]:end_split[-1] + 1]
        return entity

    def ori_tok_fn(self, batch_data, model, tokenizer, args):
        input_ids = []
        attention_mask = []
        token_type_ids = []
        span_labels_masks = []
        ...
        for item in batch_data:
            ...
            for choice in item['choices']:
                ...
                textb = item['text']
                encode_dict = tokenizer.encode_plus(texta, textb, max_length=args.max_length, padding='max_length',
                                                    truncation='longest_first')

                encode_sent = encode_dict['input_ids']
                encode_token_type_ids = encode_dict['token_type_ids']
                encode_attention_mask = encode_dict['attention_mask']
                span_label_mask = np.zeros((args.max_length, args.max_length)) - 10000
    ...
    def test_extract(self, batch_data, model, tokenizer, args):
        t0 = Timer('fn_extract', if_print=False)
        input_ids, attention_mask, token_type_ids, span_labels_mask = self.ori_tok_fn(batch_data, model, tokenizer, args)
        t0.print_time('preprocessing, about to start core')
        _, span_logits = model.model(input_ids=input_ids, attention_mask=attention_mask,
                                     token_type_ids=token_type_ids, span_labels=None, span_labels_mask=span_labels_mask)
        t0.print_time('model predict')
        # 因为原有代码会导致deprecated警告，所以修改如下：
        ...
        t0.print_time(f'after detach.. batch_data size: {len(batch_data)}')
        for i, item in enumerate(batch_data):
            '''
            优化了代码效率，并修复了一些bug：
            1.通过合理的调整程序，去掉了“text_start_id, text_end_id, offset_mapping, input_ids = self.get_actual_id(item[
            'text'], texta+'[SEP]'+textb, tokenizer, args)”。
            ...
            4.在“抽取式阅读理解”任务下，增加了top_k的选项：可在预测数据的"choices"下，增加top_k属性，如：{"entity_type": "***", 
            "top_k": 2}，若未设置top_k属性，则默认为1。
            ...
            for c in range(len(item['choices'])):
                ...
                texta = item['task_type'] + '[SEP]' + item['subtask_type'] + '[SEP]' + item['choices'][c][
                    'entity_type']
        ...
        t0.print_time(f'extraction done')
        return batch_data
    ...
    def extract(self, batch_data, model, tokenizer, args):
        ...
        for item in batch_data:
            ...
            for choice in item['choices']:
                ...
                token_type_ids0.append(encode_token_type_ids)
                span_labels_masks0.append(span_label_mask)
                t0.print_time(f'preprocessing, encoding {choice}')
        ...
        # entity_idx_span_activations = entity_idx_span_activations.cpu().numpy()
        t0.print_time(f'after detach.. batch_data size: {len(batch_data)}')
        for i, item in enumerate(batch_data):
            '''
            优化了代码效率，并修复了一些bug：
            ...
            for c in range(len(item['choices'])):
                ...
                batch_data[i]['choices'][c]['entity_list'] = entity_list
                t0.print_time(f'extracting.. c:{c}')
        return batch_data
...
class UbertPipelines:
    ...
    def pipelines_args(parent_args):
        ...
        total_parser.add_argument('--pretrained_model_path', default='IDEA-CCNL/Erlangshen-Ubert-110M-Chinese',
                                  type=str)
        total_parser.add_argument('--output_save_path', default='./predict.json', type=str)

        total_parser.add_argument('--load_checkpoints_path', default='', type=str)

        total_parser.add_argument('--max_extract_entity_number', default=1, type=float)

        total_parser.add_argument('--train', action='store_true')

        total_parser.add_argument('--threshold', default=0.5, type=float)
    ...
    def __init__(self, args):
        ...
        if args.load_checkpoints_path != '':
            self.model = UbertLitModel.load_from_checkpoint(args.load_checkpoints_path, args=args)
        else:
        ...
        self.trainer = pl.Trainer.from_argparse_args(args, logger=self.logger, callbacks=[self.checkpoint_callback])

        self.tokenizer = BertTokenizer.from_pretrained(args.pretrained_model_path, additional_special_tokens=[
            '[unused' + str(i + 1) + ']' for i in range(99)])
        self.tokenizer_fast = BertTokenizerFast.from_pretrained(args.pretrained_model_path, additional_special_tokens=[
            '[unused' + str(i + 1) + ']' for i in range(99)])
    ...
    def fit(self, train_data, dev_data):
        data_model = UbertDataModel(train_data, dev_data, self.tokenizer, self.args)
    ...
    def predict(self, predict_data, cuda=True):
        t1 = Timer('fn_pred', if_print=False)
        result = []
        start = 0
        if cuda:
            self.model = self.model
        self.model.eval()
        t1.print_time('external, model convert.')
        while start < len(predict_data):
            ...
            for k, batch_data in batch_data_bucket.items():
                t1.print_time('external, to extract')
                batch_result = self.em.extract(batch_data, self.model, self.tokenizer, self.args)
                t1.print_time('external, after extract')
                result.extend(batch_result)

        return result

    def predict_cuda(self, predict_data):
        t1 = Timer('fn_pred', if_print=False)
        result = []
        start = 0
        ...
        while start < len(predict_data):
            ...
            for k, batch_data in batch_data_bucket.items():
                batch_result = self.em.test_extract(batch_data, self.model, self.tokenizer_fast, self.args)
                t1.print_time('external, after extract')
                result.extend(batch_result)
